...
class CrawlingSpider(CrawlSpider):
    ...
    def parse(self, response):
        ...
        # headline = response.css('.entry-content::text').getall()
        # headline = headline[1].replace('\n', '').replace('\t', '').strip()
    ...
    def closed(self, reason):
        ...
        df.to_csv('dataset4.csv', mode='a', index=False, header=False)

# fetch('http://localhost:8050/render.html?url=https://businessmirror.com.ph/2024/10/27/ust-singers-madrigal-singers-loboc-childrens-choir-sing-philippines-youth-conquer-the-world/')