...
class CrawlingSpider(CrawlSpider):
    ...
    def start_requests(self):
        url = 'https://www.thesummitexpress.com/search/label/Health'
        yield SplashRequest(
            url=url,
            callback=self.parse,
            endpoint='execute',
    ...
    def parse(self, response):
        ...
        load_more_button = response.css('#load-more::attr(data-url)').get()
        if load_more_button:
...
# fetch('http://localhost:8050/render.html?url=https://www.thesummitexpress.com/search/label/Health')