...
const csvFilePath = 'scraped_data1.csv';
    ...
    try {
        ...
        for (const row of rows) {
            ...
            try {
                ...
                // Selectors
                const articleBodySelector = '.ArticleRender_article__7i2EW p';
                const dateSelector = '.ArticleHeadlineTitle_meta__eYg46 time';
                ...
                let authors = ['Unknown'];
                try {
                    authors = await page.$$eval('ul.CardTagList_tags__dheC7 a[data-component="AuthorTag"]', authorLinks =>
                        authorLinks.map(el => el.textContent.replace(/^By\s+/i, '').trim())
                    );
