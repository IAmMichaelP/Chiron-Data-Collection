            ...
            complete: async (results) => {
                ...
                for (const row of rows) {
                    ...
                    try {
                        ...
                        const articleBodySelector = '.article-body p'; // Selector for the article body
                        const authorSelector = "a.css-u1nnpx";
                        
                        await page.waitForSelector(authorSelector, { timeout: 5000 });

                        const author = await page.$eval(authorSelector, el => el.innerText.trim());
                        
                        const dateSelector = '.css-1r4r6jy';
                        const dateText = await page.$eval(dateSelector, el => el.innerText.trim());
                        ...
                        const match = dateText.match(/on\s([A-Za-z]+\s\d{1,2},\s\d{4})/);
                        const date = match ? match[1] : null;

                        // Log the article body HTML for inspection
                        console.log('Article Body HTML:', paragraphs);
                        console.log(`Article Body date and author: ${author} and ${date}`);
