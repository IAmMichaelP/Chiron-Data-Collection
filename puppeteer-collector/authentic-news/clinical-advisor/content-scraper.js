            ...
            complete: async (results) => {
                ...
                for (const row of rows) {
                    ...
                    try {
                        ...
                        const articleBodySelector = '.post-content .wysiwyg p'; // Selector for the article body
                        const authorSelector = '.author-info'; // Selector for the author information

                        // Wait for the article body selector to appear on the page
                        ...
                        let author = '';
                        try {
                            author = await page.$eval(authorSelector, (element) => element.textContent.trim());
                        } catch (error) {
                            console.error(`Could not find author information for ${columnValue}:`, error);
                            author = 'Unknown'; // Default value if author information is not found
                        }

                        // Log the article body HTML and author for inspection
                        console.log('Article Body HTML:', paragraphs);
                        console.log('Author:', author);

                        // Add the scraped content and author to the row
                        row['content'] = paragraphs.join('\n'); // Combine paragraphs into a single string
                        row['author'] = author; // Add the author to the row
                        ...
                        row['author'] = 'Unknown'; // Default value if author information is not found
                    } finally {
                        await browser.close();
                    }
                }
