            ...
            complete: async (results) => {
                ...
                for (const row of rows) {
                    ...
                    try {
                        ...
                        let date = 'Unknown';
                        try {
                            date = await page.$eval('time.entry-date', el => el.textContent.trim());
                        } catch (error) {
                            console.error(`Could not extract date:`, error);
                        }

                        // Extract the author
                        let author = 'Unknown';
                        try {
                            author = await page.$eval('p.p-meta span:first-child', el => el.textContent.replace(/^by\s+/i, '').trim());
                        } catch (error) {
                            console.error(`Could not extract author:`, error);
                        }
