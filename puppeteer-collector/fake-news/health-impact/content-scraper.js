            ...
            complete: async (results) => {
                ...
                for (const row of rows) {
                    ...
                    try {
                        ...
                        const articleBodySelector = '.post-content p'; // Selector for the article body
                        const dateSelector = '.publish-date'; // Selector for the date
                        ...
                        const { paragraphs, namedStrong } = await page.evaluate((articleBodySelector) => {
                            const elements = Array.from(document.querySelectorAll(articleBodySelector));
                            const content = [];
                            ...
                            for (const el of elements) {
                                ...
                                if (!namedStrong && el.querySelector('strong')) {
                                    const strongText = el.querySelector('strong').textContent.trim();
                                    if (strongText && /[A-Za-z]+/.test(strongText)) {
                                        namedStrong = strongText;
                                    }
                                }
                        
                                if (text.toLowerCase().startsWith("see also")) {
                                    foundSeeAlso = true;
                                    break; // Stop collecting further paragraphs
                                }
                            ...
                            return { paragraphs: content, namedStrong };
                        }, articleBodySelector);                        
                        ...
                        let author = namedStrong ? namedStrong : 'Unknown';

                        paragraphs.pop(); // remove this ending line: 'This article was written by Human Superior Intelligence (HSI)'
                        paragraphs.pop();
